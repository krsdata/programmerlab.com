( function( api ) {
	...
	api.controlConstructor['vikata-customizer-typography'] = api.Control.extend( {
		ready: function() {
			var control = this;

			control.container.on( 'change', '.vikata-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the vikata_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'vikata_get_all_google_fonts_ajax',
								vikata_customize_nonce: vikata_customize.nonce
							},
							async: false,
							dataType: 'json',
						});
						...
						if ( id in fonts ) {
							...
							jQuery( '.vikata-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.vikata-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.vikata-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.vikata-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.vikata-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.vikata-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.vikata-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.vikata-font-variant' );
			...
			control.container.on( 'change', '.vikata-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.vikata-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.vikata-font-transform select',
				function() {
...
jQuery( document ).ready( function($) {
	...
	jQuery( '.vikata-font-family select' ).selectWoo();
	jQuery( '.vikata-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".vikata-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.vikata-font-variant' ).hide();
		}
	});
